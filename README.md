...
로그인하면 역할에 따라 갈린다. 원장·강사는 `/dashboard`, 학생은 `/me`로 간다.

- **내 출결** — 월별. 날짜·상태와 함께 **등원·하원 시각**을 보여준다.
  학생이 자기가 몇 시에 왔는지 알아야 지각 판정에 수긍이 된다
- **내 납부** — 청구 주기별 청구액·납부액·상태. 미납이 있으면 합계를 위에 띄운다

급식 신청은 아직 없다. 서버는 열려 있으므로 화면만 붙이면 된다.
